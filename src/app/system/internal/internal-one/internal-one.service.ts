...
export class InternalOneService {
  ...
  public getAllEntry(params : SearchQuery) : Observable<InternalOne[]> {

  	let	link = `${this.apiConfig.host}/${this.$sa}/entries`;

    let httpOptions = {'params' : params};

    return this.http.get<InternalOne[]>(link , httpOptions)
  ...
  public getEntry(entry : string) : Observable<InternalOne> {
  ...
  public addEntry$(internal : InternalOne) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/create`;

    return this.http.post<InternalOne>(link , internal)

      .pipe(

          tap((entry : General) => console.log(entry)) ,

          map((entry : InternalOne) => { return {'created' : true , '$data' : entry}; }) ,

          catchError(this.handleError<InternalOne>(`${this.$systemType} Entry` , null))
        )
  }

  public updateEntry(entry : string) : Observable<InternalOne> {
  ...
  public updateEntry$(entry : string , body : InternalOne) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/${entry}/update`;

  return this.http.put<InternalOne>(link , body)

      .pipe(

        map((entry : InternalOne) => { return { 'updated' : true , '$data' : entry }; }) ,
  ...
  public $deleteManyEntry(arrayIdx : number[]) : Observable<General> {
    ...
    return this.http.patch(link , body)
      ...
      .pipe(
        ...
        catchError(this.handleError<InternalOne[]>(`${this.$systemType} Entries Delete` , null))
  ...
  public $deleteAllInternalOne() : Observable<General> {

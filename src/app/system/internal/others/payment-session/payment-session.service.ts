...
export class PaymentSessionService {
  ...
  public getAllEntry(params : SearchQuery) : Observable<PaymentSession[]> {

  	let	link = `${this.apiConfig.host}/${this.$sa}/entries`;

    let httpOptions = {'params' : params};

    return this.http.get<PaymentSession[]>(link , httpOptions)

      .pipe(
              tap((value : PaymentSession[]) => console.log(value)) ,

              delay(2000) ,

              catchError(this.handleError<PaymentSession[]>(`${this.$systemType} Entries` , null))
        )

  }

  public getEntry(entry : string) : Observable<PaymentSession> {

  	let	link = `${this.apiConfig.host}/${this.$sa}/entry/${entry}/detail`;

  	return this.http.get<PaymentSession>(link)
       
       .pipe(

          tap((entry : PaymentSession) => console.log(entry)) ,

          delay(2000) ,

          catchError(this.handleError<PaymentSession>(`${this.$systemType} Entry` , null))
        )
  }
  ...
  public addEntry$(internal : PaymentSession) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/create`;

    return this.http.post<PaymentSession>(link , internal)

      .pipe(

          tap((entry : General) => console.log(entry)) ,

          map((entry : PaymentSession) => { return {'created' : true , '$data' : entry}; }) ,

          catchError(this.handleError<PaymentSession>(`${this.$systemType} Entry` , null))
        )
  }

  public updateEntry(entry : string) : Observable<PaymentSession> {
  ...
  public updateEntry$(entry : string , body : PaymentSession) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/${entry}/update`;

  return this.http.put<PaymentSession>(link , body)

      .pipe(

        map((entry : PaymentSession) => { return { 'updated' : true , '$data' : entry }; }) ,
  ...
  public $deleteManyEntry(arrayIdx : number[]) : Observable<General> {
    ...
    return this.http.patch(link , body)
      ...
      .pipe(
        ...
        catchError(this.handleError<PaymentSession[]>(`${this.$systemType} Entries Delete` , null))
  ...
  public deleteAllEntry() : Observable<any> {
    ...
    return this.http.get(link)
      ...
      .pipe(
        ...
        catchError(this.handleError<PaymentSession[]>(`${this.$systemType} Entry or Entries Delete` , null))

        );
  }

  public $deleteAllPaymentSession() : Observable<General> {

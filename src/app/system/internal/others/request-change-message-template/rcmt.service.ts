...
export class RequestChangeMessageTemplateService {
  ...
  public getAllEntry(params : SearchQuery) : Observable<RequestChangeMessageTemplate[]> {

  	let	link = `${this.apiConfig.host}/${this.$sa}/entries`;

    let httpOptions = {'params' : params};

    return this.http.get<RequestChangeMessageTemplate[]>(link , httpOptions)

      .pipe(
              tap((value : RequestChangeMessageTemplate[]) => console.log(value)) ,

              delay(2000) ,

              catchError(this.handleError<RequestChangeMessageTemplate[]>(`${this.$systemType} Entries` , null))
        )

  }

  public getEntry(entry : string) : Observable<RequestChangeMessageTemplate> {

  	let	link = `${this.apiConfig.host}/${this.$sa}/entry/${entry}/detail`;

  	return this.http.get<RequestChangeMessageTemplate>(link)
       
       .pipe(

          tap((entry : RequestChangeMessageTemplate) => console.log(entry)) ,

          delay(2000) ,

          catchError(this.handleError<RequestChangeMessageTemplate>(`${this.$systemType} Entry` , null))
        )
  }
  ...
  public addEntry$(internal : RequestChangeMessageTemplate) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/create`;

    return this.http.post<RequestChangeMessageTemplate>(link , internal)

      .pipe(

          tap((entry : General) => console.log(entry)) ,

          map((entry : RequestChangeMessageTemplate) => { return {'created' : true , '$data' : entry}; }) ,

          catchError(this.handleError<RequestChangeMessageTemplate>(`${this.$systemType} Entry` , null))
        )
  }

  public updateEntry(entry : string) : Observable<RequestChangeMessageTemplate> {
  ...
  public updateEntry$(entry : string , body : RequestChangeMessageTemplate) : Observable<General> {

    let  link = `${this.apiConfig.host}/${this.$sa}/entry/${entry}/update`;

  return this.http.put<RequestChangeMessageTemplate>(link , body)

      .pipe(

        map((entry : RequestChangeMessageTemplate) => { return { 'updated' : true , '$data' : entry }; }) ,
  ...
  public $deleteManyEntry(arrayIdx : number[]) : Observable<General> {
    ...
    return this.http.patch(link , body)
      ...
      .pipe(
        ...
        catchError(this.handleError<RequestChangeMessageTemplate[]>(`${this.$systemType} Entries Delete` , null))
  ...
  public deleteAllEntry() : Observable<any> {
    ...
    return this.http.get(link)
      ...
      .pipe(
        ...
        catchError(this.handleError<RequestChangeMessageTemplate[]>(`${this.$systemType} Entry or Entries Delete` , null))

        );
  }

  public $deleteAllRequestChangeMessageTemplate() : Observable<General> {

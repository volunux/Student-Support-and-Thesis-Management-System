...
export class AttachmentUploadComponent implements OnInit , AfterViewInit , OnDestroy , OnChanges {
  ...
  constructor(public aus : AttachmentUploadService , public ems : ErrorMessagesService) { 
  ...
  public $fileObj = {	'key' : '' , 'attachment' : { 'location' : '' , 'size' : false , 'mimetype' : '' } };
  ...
  @Input('end-points') public endpoints : { [key : string] : any };
  ...
  ngOnDestroy() : void {
  	...
  	if (this.removeFailed) this.removeFailed.unsubscribe(); 
...
public $checkAttachmentSize($myFile , $fileRef , $btnRef? : ElementRef) {
	...
	public $checkAttachmentType($myFile : File , $fileRef : { [key : string] : any } , $btnRef? : ElementRef) {
		...
		$fileRef.attachment.type = true;
			...
			else { $btnRef.nativeElement.textContent = 'Choose a file to upload.';

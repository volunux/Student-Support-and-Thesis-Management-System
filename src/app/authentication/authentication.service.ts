...
export class AuthenticationService {
  ...
  public getCurrentUser() : User {
  		...
  		if (this.isLoggedIn()) {
  				...
  				const { email_address , username , role , department , faculty , status } = JSON.parse(atob(token.split('.')[1]));

  				return { email_address , username , role , department , faculty , status } as User;
  		}
  }
